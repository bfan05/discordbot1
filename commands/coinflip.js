...
module.exports = {
    ...
    async execute(client, message, args, Discord, profileData) {
        ...
        if (args[0] > data.coins) return message.channel.send("You don't have enough TMA Cash, silly!");
        if (data.coins < 0 || args[0] <= 0) return message.channel.send("You must gamble a positive amount, silly!");
        ...
        if (flip == 25) {
            ...
            if (args[1] == 'heads' || args[1] == 'h') {
                update = bet;
                message.channel.send(`**${message.author.username}** bet **${bet.toLocaleString()}** and chose heads... 🪙 **|** the coin lands on **heads**! You won **${(2 * bet).toLocaleString()}** TMA Cash!`);
            ...
            if (args[1] == 'tails' || args[1] == 't') {
                update = bet;
                message.channel.send(`**${message.author.username}** bet **${bet.toLocaleString()}** and chose tails... 🪙 **|** the coin lands on **tails**! You won **${(2 * bet).toLocaleString()}** TMA Cash!`);
                //client.channels.cache.get('857330940631187526').send(`**${message.author.username} (${message.author.id})** bet **${bet.toLocaleString()}** and won **${(2 * bet).toLocaleString()}**.`);
                client.channels.cache.get('857698338051588127').send(`**${message.author.username} (${message.author.id})** bet **${bet.toLocaleString()}** and won **${(2 * bet).toLocaleString()}**.`);
            }
            else {
